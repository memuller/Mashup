ActionController::Routing::Routes.draw do |map|
  ...
  map.language '/:locale/:tag', :controller => "tag", :requirements => {:locale => /es|en/}
	...
	map.tag '/:tag.:format' , :controller => "tag", :requirements => { :tag => /.+(?=\.(html|xhtml|rss))|.+/ } 

	map.media_tag '/:action/tag/:tag.:format' , :tag => nil, 	:controller => "tag", :requirements => {:action => /blog|news|video|photo|microblog|bookmark|timeline|cooliris/,  :tag => /.+(?=\.(html|xhtml|rss))|.+/ }
